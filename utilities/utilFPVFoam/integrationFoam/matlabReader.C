...
:  composition(composition),
   ...
   pvIndex_param_(matlabDict.lookup("pvIndex_param")),
   f_param_(matlabDict.lookup("f_param"))
{
   ...
   tableNames_.append("PV"); //added by jianhong, 02/12/2019
   tableNames_.append("TotalEnthalpy"); //added by jianhong,25/12/2019
   ...
   for (int i=0; i<readData_.size(); i++)
       {
   	   readData_[i].resize(pvIndex_param_.size());
   ...
   for (int i=0; i<sampledData_.size(); i++)
       {
            sampledData_[i].resize(pvIndex_param_.size());
   ...
   // Integration
       ...
       // for all flamelets
       for (int numPV=0; numPV<pvIndex_param_.size();numPV++)
       {
    	   Info << "Progress: " << 100*numPV/(pvIndex_param_.size()-1) << " %" <<  endl;
          // for all species
