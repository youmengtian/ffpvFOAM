...
:
    ...
    transportVarf_(lookupOrDefault<Switch>("transportVarf", false)),
...
void LESModel::correctVarf()
{
	...
	{
	    varf_ = Cvarf_ * sqr(delta()) * magSqr(fvc::grad(this->thermo().f()));
	    varf_.correctBoundaryConditions();
	}
...
void LESModel::correctChi()
{
	if (transportVarf_)
	{
	    Chi_ = mu()/(Sc_*rho_)*magSqr(fvc::grad(this->thermo().f())) + Cchi_*DfSgs()*varf_/(2.0*rho_*sqr(delta()));
	    Chi_.correctBoundaryConditions();
	}
	else
	{
	    Chi_ = Cchi_ * DfEff()/rho_ * magSqr(fvc::grad(this->thermo().f()));
	    Chi_.correctBoundaryConditions();
	}
