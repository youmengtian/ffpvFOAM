...
#include "FPVunnormalizedModel.H"
...
FPVunnormalizedModel<CombThermoType, ThermoType>::FPVunnormalizedModel
(
...
template<class CombThermoType, class ThermoType>
hashedWordList FPVunnormalizedModel<CombThermoType, ThermoType>::tables()
{
	hashedWordList tableNames = this->thermo().composition().species();
    //if(this->thermo().he().name() == "h"){tableNames.append("Sh");};
	tableNames.append("T");
...
template<class CombThermoType, class ThermoType>
void FPVunnormalizedModel<CombThermoType, ThermoType>::correct()
{
//Info << "codeis here 000" <<endl;
...
//    scalarField& PVNormCells = PVNorm_.internalField(); //comment out by jianhong

    //- Update the species and temperature field
    if(this->active())
    {
       scalarList x(3, 0.0);
 //      double PVmax;   	//comment out by jianhong
       ...
       forAll(Y_, i)
       {
          ...
          forAll(fCells, cellI)
          {
           ...
           //      PVmax = solver_.interp1D(fCells[cellI], PVmaxTable_);   	//comment out by jianhong 
          //       PVNormCells[cellI] = max(min(PVCells[cellI]/max(PVmax,1e-10),1),0);  //comment out by jianhong, not to normalize
                 x[0] = PVCells[cellI];
...
//				  Info <<cellI << " 0 useMixtureFractionVariance " <<" "<<  useMixtureFractionVariance_<< " " << x[1] << endl;	
...
//				  Info <<cellI << " 1 useMixtureFractionVariance " <<" "<<  useMixtureFractionVariance_<< " " << x[1] << endl;	
                 ...
                 //Lookup
                 TCells[cellI] = solver_.interpolate(ubIF_[cellI], posIF_[cellI], (solver_.sizeTableNames() - 5));
//				 Info << cellI <<" x[0] =" << x[0] <<" x[1] =" << x[1] <<" x[2] =" << x[2] << " in Tcells = " << TCells[cellI] << endl;

                 PVsCells[cellI] = solver_.interpolate(ubIF_[cellI], posIF_[cellI], (solver_.sizeTableNames() - 4));
...
//				 Info << cellI <<" x[0] =" << x[0] <<" x[1] =" << x[1] <<" x[2] =" << x[2] << " in muCells = " << muCells[cellI] << endl;

                 alphaCells[cellI] = solver_.interpolate(ubIF_[cellI], posIF_[cellI], (solver_.sizeTableNames() - 2));
//				 Info << cellI <<" x[0] =" << x[0] <<" x[1] =" << x[1] <<" x[2] =" << x[2] << " in alphaCells = " << alphaCells[cellI] << endl;

                 HRsCells[cellI] = solver_.interpolate(ubIF_[cellI], posIF_[cellI], (solver_.sizeTableNames() - 1));
//				 Info << cellI <<" x[0] =" << x[0] <<" x[1] =" << x[1] <<" x[2] =" << x[2] << " in HRsCells = " << HRsCells[cellI] << endl;
...
//          fvPatchScalarField& pPVNorm = PVNorm_.boundaryField()[patchi];  	//comment out by jianjing
              ...
              forAll(pY , facei)
              {
             	 if (i == 0)
             	 {
                   ...
                   //  pPVNorm[facei] = max(min(pPV[facei]/max(PVmax,1e-10),1),0);
                     ...
                     pT[facei] = solver_.interpolate(ubP_[facei], posP_[facei], (solver_.sizeTableNames() - 5));
                     pPVs[facei] = solver_.interpolate(ubP_[facei], posP_[facei], (solver_.sizeTableNames() - 4));
                     pmu[facei] = solver_.interpolate(ubP_[facei], posP_[facei], (solver_.sizeTableNames() - 3));
                     palpha[facei] = solver_.interpolate(ubP_[facei], posP_[facei], (solver_.sizeTableNames() - 2));
...
template<class CombThermoType, class ThermoType>
void FPVunnormalizedModel<CombThermoType, ThermoType>::correctCurrentFields
(
...
FPVunnormalizedModel< CombThermoType, ThermoType>::getHRs() const
{
...
template<class CombThermoType, class ThermoType>
Foam::tmp<Foam::volScalarField>
FPVunnormalizedModel< CombThermoType, ThermoType>::getT() const
{
...
template<class CombThermoType, class ThermoType>
Foam::tmp<Foam::volScalarField>
FPVunnormalizedModel< CombThermoType, ThermoType>::getPVs() const
{
...
template<class CombThermoType, class ThermoType>
Foam::tmp<Foam::volScalarField>
FPVunnormalizedModel< CombThermoType, ThermoType>::getYi(const label& i) const
{
...
template<class CombThermoType, class ThermoType>
Switch FPVunnormalizedModel<CombThermoType, ThermoType>::correctDensity()
{
	return true;
}
...
FPVunnormalizedModel<CombThermoType, ThermoType>::R
(
    volScalarField& Y              
) const
{
...
template<class CombThermoType, class ThermoType>
Foam::tmp<Foam::volScalarField>
FPVunnormalizedModel< CombThermoType, ThermoType>::Sh() const
{
...
template<class CombThermoType, class ThermoType>
Foam::tmp<Foam::volScalarField>
FPVunnormalizedModel< CombThermoType, ThermoType>::dQ() const
{
...
template<class CombThermoType, class ThermoType>
bool FPVunnormalizedModel<CombThermoType, ThermoType>::read()
{
